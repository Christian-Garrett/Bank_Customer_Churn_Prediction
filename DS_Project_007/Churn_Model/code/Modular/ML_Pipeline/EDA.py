...
def get_descriptive_stats(self):
    ...
    print('\n', self.data.shape)
    print('\n', self.data.head(10).T)
    print('\n', self.data.describe()) # Describe numerical columns
    print('\n', self.data.describe(include = ['O'])) # Describe categorical columns
    print(f'\nThere are {self.data.shape[0]} records and \
{self.data.CustomerId.nunique()} unique customers')

    surname_stats = \
        self.data.groupby(['Surname']
                          ).agg({'RowNumber':'count', 
                                 'Exited':'mean'}
...
def univariate_visualizations(self):
    ...
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/credit_score_boxplot.png"))
    plt.clf()

    ## Age
    sns.boxplot(y=self.data.Age)
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/age_boxplot.png"))
    plt.clf()

    ## Tenure
    sns.violinplot(y=self.data.Tenure)
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/tenure_violinplot.png"))
    plt.clf()

    ## Balance
    sns.violinplot(y=self.data.Balance)
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/balance_violinplot.png"))
    plt.clf()
    ...
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/numProducts_distplot.png"))
    plt.clf()

    ## EstimatedSalary
    sns.kdeplot(self.data.EstimatedSalary)
    plt.savefig(os.path.join(self.output_path,
                             "Univariate_Analysis/estimatedSalary_kdeplot.png"))
    plt.clf()
 

def bivariate_visualizations(self):
    
    ## Linear correlation between features and the target 
    corr = self.data.corr()
    print(corr)
    ...
    plt.savefig(os.path.join(self.output_path,
                             "Bivariate_Analysis/train_corr_heatmap.png"))
    plt.clf()   

    ## Churn vs Age Boxplot  
    sns.boxplot(x="Exited", y="Age", data=self.data, palette="Set3")
    plt.savefig(os.path.join(self.output_path,
                             "Bivariate_Analysis/age_churn_boxplot.png"))
    plt.clf()
    ...
    for col in ['Gender',
                ...
                'country_France',
                'NumOfProducts']:
        print(f"Value Counts (training): \
{self.train_data.groupby([col]).Exited.mean()}")
